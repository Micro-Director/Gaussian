#!/share/home/ruoting/anaconda3/bin/python
#-*-coding:utf-8-*-
import os
import sys
import numpy as np
...
def check_scf(log_file):


    if os.popen("grep 'Convergence criterion not met' " + log_file).read().split('\n')[0]:

        print ("SCF is not complete, please modify the parameters !") 


def ExactSCFItera(log_file):

    IteraLine = np.array(os.popen("grep -n 'Cycle[[:blank:]][0-9]*' " + log_file).read().split('\n'))[:-1]
    IteraLine = np.array([i.split(':')[0][:] for i in IteraLine]).astype(int)
    E = np.zeros(shape=(0,), dtype=float)
    DeltaE = np.zeros(shape=(0,), dtype=float)
    RMSDP = np.zeros(shape=(0,), dtype=float)
    MAXDP = np.zeros(shape=(0,), dtype=float)
    for i_index, i in enumerate(IteraLine[1:]):
        E = np.append(E, float(os.popen("sed -n " + str(IteraLine[i_index+1] + 1) + "p " + log_file).read().split()[1]), )
        DeltaE = np.append(DeltaE, float(os.popen("sed -n " + str(IteraLine[i_index+1] + 1) + "p " + log_file).read().split()[3]), )
        if i_index != len(IteraLine[1:])-1:
            t = os.popen("sed -n " + str(IteraLine[i_index+1]) + ',' + str(IteraLine[i_index+2]) + 'p ' + log_file + "| grep 'RMSDP'").read().split('\n')[0]
            RMSDP = np.append(RMSDP, float(t.split('=')[1].split()[0].replace('D','e')))
            MAXDP = np.append(MAXDP, float(t.split('=')[2].split()[0].replace('D','e')))
        else:
            t = os.popen("sed -n " + str(IteraLine[i_index+1]) + ',' + str(IteraLine[i_index+1]+100) + 'p ' + log_file + "| grep 'RMSDP'").read().split('\n')[0]
            RMSDP = np.append(RMSDP, float(t.split('=')[1].split()[0].replace('D','e')))
            MAXDP = np.append(MAXDP, float(t.split('=')[2].split()[0].replace('D','e')))
    ...
    fig, ax = plt.subplots(nrows = 2, ncols = 2, figsize = (16, 14))
    ax1 = plt.subplot(221)
    ax1.scatter(range(len(E)), E, s= 150, color = 'none', marker='o',edgecolors='darkblue', alpha = 0.8, linewidths=3)
    ax1.plot(range(len(E)), E, '-', color = 'darkblue', linewidth=3.0, )
    ax1.set_ylim(1.0000001*min(E), 0.999999*min(E))
    ax1.set_xlabel('Iteration step', fontsize = 25, fontweight = 'semibold')
    ax1.set_ylabel('Energy (eV)', fontsize = 25, fontweight = 'semibold', color = 'darkblue')
    ax1.tick_params(axis = 'both', labelsize = 25, width = 2, length = 8, direction='in')
    ...
    ax2 = plt.subplot(222)
    ax2.scatter(range(len(DeltaE)), DeltaE, s= 150, color = 'none', marker='o',edgecolors='darkblue', alpha = 0.8, linewidths=3)
    ax2.plot(range(len(DeltaE)), DeltaE, '-', color = 'darkblue', linewidth=3.0, )
    # ax2.plot(range(len(RMSForce)), [RFTh]*len(RMSForce), '--', color = 'black', linewidth=2.0, )
    # ax1.set_xlim(np.min(self._xkpt), np.max(self._xkpt))
    ax2.set_ylim(-0.00001, 0.00001)
    ax2.set_xlabel('Iteration step', fontsize = 25, fontweight = 'semibold')
    ax2.set_ylabel('Delta Energy (eV)', fontsize = 25, fontweight = 'semibold', color = 'darkblue')
    ax2.tick_params(axis = 'both', labelsize = 25, width = 2, length = 8, direction='in')
    ...
    ax3 = plt.subplot(223)
    ax3.scatter(range(len(RMSDP)), RMSDP, s= 150, color = 'none', marker='o',edgecolors='darkblue', alpha = 0.8, linewidths=3)
    ax3.plot(range(len(RMSDP)), RMSDP, '-', color = 'darkblue', linewidth=3.0, )
    # ax3.plot(range(len(MaxDis)), [MDTh]*len(MaxDis), '--', color = 'black', linewidth=2.0, )
    # ax1.set_xlim(np.min(self._xkpt), np.max(self._xkpt))
    ax3.set_ylim(0, 100*min(RMSDP))
    ax3.set_xlabel('Iteration step', fontsize = 25, fontweight = 'semibold')
    ax3.set_ylabel('RMS DP', fontsize = 25, fontweight = 'semibold', color = 'darkblue')
    ax3.tick_params(axis = 'both', labelsize = 25, width = 2, length = 8, direction='in')
    ...
    ax4 = plt.subplot(224)
    ax4.scatter(range(len(MAXDP)), MAXDP, s= 150, color = 'none', marker='o',edgecolors='darkblue', alpha = 0.8, linewidths=3)
    ax4.plot(range(len(MAXDP)), MAXDP, '-', color = 'darkblue', linewidth=3.0, )
    # ax4.plot(range(len(RMSDis)), [RDTh]*len(RMSDis), '--', color = 'black', linewidth=2.0, )
    # ax1.set_xlim(np.min(self._xkpt), np.max(self._xkpt))
    ax4.set_ylim(0, 100*min(MAXDP))
    ax4.set_xlabel('Iteration step', fontsize = 25, fontweight = 'semibold')
    ax4.set_ylabel('MAX DP', fontsize = 25, fontweight = 'semibold', color = 'darkblue')
    ax4.tick_params(axis = 'both', labelsize = 25, width = 2, length = 8, direction='in')
